...
public:
    ...
    QString changeLog(const StrategyConfig& d)const
    {
        ...
        {
            t += QString("SensorIDs:");
            foreach(auto id, lSensorIDs)
            {
                t += QString("%1,").arg(id);
            }
            t += QString(" -> ");
            foreach(auto id, d.lSensorIDs)
            {
                t += QString("%1,").arg(id);
            }
            t += QString("\n");
    ...
    QList<QString> lSensorIDs;               //策略影响的缺陷ID列表
};
...
public:
    bool operator!=(const SystemConfig& d)const
    {
        return iTimeInterval_GeneRecord != d.iTimeInterval_GeneRecord ||
            ...
            iProtocolVersion != d.iProtocolVersion;
    }
    ...
    QString changeLog(const SystemConfig& d)const
    {
        ...
        if (iProtocolVersion != iProtocolVersion)
        {
            t += QString("iProtocolVersion:%1 -> %2\n").arg(iProtocolVersion).arg(d.iProtocolVersion);
        }
        return t;
    }
    ...
    int iProtocolVersion;               //协议版本, 0:VEXI的版本,1:2.4版本用于MCAL(TERA);
};
